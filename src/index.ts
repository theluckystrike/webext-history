...
export async function searchHistory(
  ...
  return chrome.history.search(query as chrome.history.HistoryQuery);
}
 ...
 */
export async function getVisits(url: string): Promise<chrome.history.VisitItem[]> {
 ...
 */
export async function deleteRange(startTime: number, endTime: number): Promise<void> {
 ...
 */
export function onVisited(
  callback: (result: chrome.history.HistoryItem) => void
): () => void {
  const listener = (result: chrome.history.HistoryItem) => {
    callback(result);
  };
 ...
 * @returns Unsubscribe function
 */
export function onVisitRemoved(
  callback: (removed: { allHistory: boolean; urls?: string[] }) => void
): () => void {
  const listener = (removed: chrome.history.RemovedResult) => {
    callback(removed);
  };
